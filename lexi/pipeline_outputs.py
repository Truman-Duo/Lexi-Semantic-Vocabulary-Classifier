"""Output format generators for the classification pipeline."""
...
from .models import WordInfo, CategoryMap


def build_category_map(word_infos: Dict[str, WordInfo]) -> CategoryMap:
...
def generate_markdown(classified: CategoryMap, output_md: str):
...
def generate_json(classified: CategoryMap, output_json: str,
...
def generate_csv(classified: CategoryMap, word_infos: Dict[str, WordInfo],
                 output_csv: str):
    import csv
    rows = []
    for main_cat, sub_cats in classified.items():
...
def generate_html(classified: CategoryMap, word_infos: Dict[str, WordInfo],
                  output_html: str):
    cats_data = {}
    for main_cat, sub_cats in classified.items():
        cats_data[main_cat] = {}
...
def generate_anki(classified: CategoryMap, word_infos: Dict[str, WordInfo],
                  output_anki: str):
    try:
        import genanki
    except ImportError:
        ...
        for sub_cat, words in sub_cats.items():
            for wi in words:
                ...
                note = genanki.Note(
                    model=model,
                    fields=[
                        wi.word, main_cat, sub_cat,
                        wi.cefr_level, str(round(wi.zipf_frequency, 2)),
                    ])
                deck.add_note(note)
